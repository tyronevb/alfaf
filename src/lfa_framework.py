...
class AutomatedLFAFramework(object):
    """Implement the Automated Log File Analysis Framework."""

    def __init__(
        self,
        data_miner_config: str = None,
        ...
        self.name = name
        self.output_dir = "{base}{name}_{timestamp}/".format(
            base=output_dir, name=self.name, timestamp=datetime.datetime.now().strftime("%d-%m-%y_%H_%M_%S")
        )
        self.training_mode = True if mode == "training" else False

        # instantiate DataMiner
        self.data_miner = DataMiner(
            config_file=data_miner_config, input_dir=self.input_dir, output_dir=self.output_dir, verbose=True
        )

        # instantiate InferenceEngine for either predicting or training -> defined by config file
        self.inference_engine = InferenceEngine(
            config_file=inference_engine_config, name=self.name, device=device, output_dir=self.output_dir, verbose=True
        )
    ...
    def analyse_log_file(self, input_log_file_name: str, verbose: bool) -> None:
        ...
        Generates a debugging report identifying potential anomalies lines in a provided log file.
        ...
        debug_report = self._generate_debug_report(
            df_parsed_log=self.df_parsed_log, df_translated_ad_report=translated_ad_report
        )
        ...
        output_path = "{directory}{name}_{timestamp}_debug_report".format(
            directory=self.output_dir, name=self.name, timestamp=datetime.datetime.now().strftime("%d-%m-%Y-%Hh%Mm%Ss")
        )
    ...
    def _data_preparation(self, input_log_file_name: str) -> DataLoader or pd.DataFrame:
        ...
        """
        # parse logs
        path_to_structured_log, path_to_event_matrix = self.data_miner.parse_logs(
            log_file=input_log_file_name, save_parameters=True
        )
    ...
    def _create_datastore(self, log_events_and_keys: str) -> None:
        ...
        output_path = "{directory}{name}_{timestamp}_datastore.csv".format(
            directory=self.output_dir, name=self.name, timestamp=datetime.datetime.now().strftime("%d-%m-%Y-%Hh%Mm%Ss")
        )
    ...
    def _prepare_new_events_for_datastore(df_new_log_events: pd.DataFrame) -> pd.DataFrame:
        """
        Prepare new, unseen log events for entry into the datastore.
    ...
    def _update_datastore(self, datastore: str, df_new_log_events: pd.DataFrame) -> None:
        """
        Update the Log Key - Log Event Template Data Store with new log keys and events.
    ...
    def _translate_to_log_keys(self, df_anomaly_detection_report: pd.DataFrame) -> pd.DataFrame:
        """
        Translate the generated anomaly detection report to log keys from features.
        ...
        reverse_transformation = {
            value: key for key, value in self.inference_engine.feature_extractor.data_transformation.items()
        }
